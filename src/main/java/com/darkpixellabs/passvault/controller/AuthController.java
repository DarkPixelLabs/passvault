...
public class AuthController {
    ...
    public ResponseEntity<?> setupStatus() {
        return ResponseEntity.ok(Map.of("configured", userRepository.count() > 0));
    }

    @PostMapping("/setup")
    public ResponseEntity<?> setup(@RequestBody PasswordRequest request, HttpServletRequest httpRequest) {
        String ip = clientIp(httpRequest);
        if (!rateLimiter.allow(ip)) return ResponseEntity.status(429).body(Map.of("error", "Too many attempts. Try again later."));
        if (userRepository.count() > 0) return ResponseEntity.status(409).body(Map.of("error", "Master password is already configured."));
        ...
        try {
            ...
            user.setEncryptionSalt(cryptoService.generateSalt());
    ...
    public ResponseEntity<?> login(@RequestBody PasswordRequest request, HttpServletRequest httpRequest) {
        String ip = clientIp(httpRequest);
        if (!rateLimiter.allow(ip)) return ResponseEntity.status(429).body(Map.of("error", "Too many attempts. Try again later."));
        if (request == null || request.masterPassword() == null) return ResponseEntity.status(401).body(Map.of("error", "Invalid master password."));

        VaultUser user = userRepository.findAll().stream().findFirst().orElse(null);
        if (user == null) return ResponseEntity.status(401).body(Map.of("error", "Invalid master password."));
        char[] verifyPassword = request.masterPassword().toCharArray();
        boolean valid;
        try { valid = cryptoService.verifyMasterPassword(verifyPassword, user.getPasswordHash()); }
        catch (RuntimeException e) { valid = false; }
        if (!valid) return ResponseEntity.status(401).body(Map.of("error", "Invalid master password."));
        ...
        try {
            ...
            ResponseCookie cookie = ResponseCookie.from(SESSION_COOKIE, token).httpOnly(true).secure(false)
                    .sameSite("Strict").path("/").maxAge(SESSION_COOKIE_MAX_AGE).build();
    ...
    public ResponseEntity<?> logout(HttpServletRequest request, HttpServletResponse response) {
        sessionManager.invalidate(extractSessionToken(request));
        ResponseCookie cookie = ResponseCookie.from(SESSION_COOKIE, "").httpOnly(true).secure(false)
    ...
    private static String extractSessionToken(HttpServletRequest request) {
        ...
        for (var cookie : request.getCookies()) if (SESSION_COOKIE.equals(cookie.getName())) return cookie.getValue();
        return null;
    }

    private static String clientIp(HttpServletRequest request) { return request.getRemoteAddr(); }
